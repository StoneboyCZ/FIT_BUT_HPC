function [t,y,ORD,DY_all]=explicitTaylorMult_GNPV_ver1(h,tspan,init,A,A2,A3,A4,A5,b,ij,ijk,ijkl,ijklm,eps,ind,maxORD)
...
% VERSION 1: recurrent calculation of Taylor series terms among the same
...
%     end
    ...
    % computation of DY1 (indeces 1 1 1 ... ) 
    DY_3terms(:,1) = DY(ijk(:,2),1).*DY(ijk(:,3),1);
    DY_4terms(:,1) = DY(ijkl(:,2),1).*DY(ijkl(:,3),1).*DY(ijkl(:,4),1);
    DY_5terms(:,1) = DY(ijklm(:,2),1).*DY(ijklm(:,3),1).*DY(ijklm(:,4),1).*DY(ijklm(:,5),1);
...
%         r3 = ind.DY_ijk{3,k-1}(i3_new_start:end)
        ...
        % are always ones 
        DY_4terms(:,k) = sum(DY(j4,ind.DY_ijkl{2,k-1}(t4_new_start:end)).*DY(k4,ind.DY_ijkl{3,k-1}(t4_new_start:end)).*DY(l4,ind.DY_ijkl{4,k-1}(t4_new_start:end)),2);
        ...
        % rows from DY, we do not consider DY_ijk indeces, because there
        % are always ones 
        DY_5terms(:,k) = sum(DY(j5,ind.DY_ijklm{2,k-1}(t5_new_start:end)).*DY(k5,ind.DY_ijklm{3,k-1}(t5_new_start:end)).*DY(l5,ind.DY_ijklm{4,k-1}(t5_new_start:end)).*DY(m5,ind.DY_ijklm{5,k-1}(t5_new_start:end)),2);
...
%         A5_diff = (A5y - A5y_GN)'
    ...
    if k > maxORD % max ORD was reached
        ...
        fprintf('GN v1: Halving integration step size. %d\n',t(i-1))
    else
%         DY_all{i-1}=DY;
%         if k<(stopping+4)
%             h=h*2;
