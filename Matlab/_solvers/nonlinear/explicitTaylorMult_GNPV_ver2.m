function [t,y,ORD,DY_all]=explicitTaylorMult_GNPV_ver2(h,tspan,init,A,A2,A3,A4,A5,b,ij,ijk,ijkl,ijklm,eps,ind,maxORD)
...
%         
         DY(:,k+1)=(h/k)*(Ay+A2y+A3y+A4y+A5y);
         %DY(:,k+1)=(h/k)*sum(Ay,A2y,A3y,A4y,A5y,2);
         %GNPV = DY(:,1:k+1)
         %return
%         GNPV = DY(:,1:k+1)
%         
...
%         A5_diff = (A5y - A5y_GN)'
    ...
    if k > maxORD % max ORD was reached
        ...
        fprintf('GN v2: Halving integration step size. %d\n',i)
    else
%         DY_all{i-1}=DY;
%         if k<(stopping+4)
%             h=h*2;
