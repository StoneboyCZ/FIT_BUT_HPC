function [t,y,ORD,DY_all]=explicitTaylorMult(h,tspan,init,A,A2,A3,A4,A5,b,ij,ijk,ijkl,ijklm,eps)
...
%     maxDY = ones(1,stopping)*10^10;
    ...
    while norm(maxDY(k-1:stopping+k-2))>eps
        ...
        mm1=k:-1:1;
        mm2=1:k;
        A2y=A2*sum(DY(ij(:,1),mm1).*DY(ij(:,2),mm2),2);
        
        % cubatic combinations term
        A3y=zeros(length(ijk(:,1)),1);
        for jj=1:k
            mm1=k+1-jj:-1:1;
            mm2=(1:k-jj+1);
        ...
        A4y=zeros(length(ijkl(:,1)),1);
        for ii=1:k
            A4ysum_ii=zeros(length(ijkl(:,1)),1);
            for jj=1:k-ii+1
                mm1=k+2-jj-ii:-1:1;
                mm2=(1:k-jj-ii+2);
        ...
        A5y=zeros(length(ijklm(:,1)),1);
        for iii=1:k
            A5ysum_iii=zeros(length(ijklm(:,1)),1);            
            for ii=1:k-iii+1
                A5ysum_ii=zeros(length(ijklm(:,1)),1);
                for jj=1:k-ii-iii+2
