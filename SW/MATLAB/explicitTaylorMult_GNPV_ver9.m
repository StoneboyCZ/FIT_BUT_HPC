function [t,y,ORD,DY_all]=explicitTaylorMult_GNPV_ver9( ... 
    ...
    hScaleFactor ... how h is scaled when ORD is increased
    )
...
% VERSION 9: !!!! EXPERIMENTAL VERSION !!!! same equations in ij** optimizations  
...
%             DY_5terms_smaller(:,k) = sum(DY(j5,ind.DY_ijklm{2,k-1}(t5_new_start_smaller:end)).*DY(k5,ind.DY_ijklm{3,k-1}(t5_new_start_smaller:end)).*DY(l5,ind.DY_ijklm{4,k-1}(t5_new_start_smaller:end)),2); 
         ...
         %fprintf('i: %d Ay: %g, A2y: %g , A3y: %g , A4y: %g \n',i,Ay,A2y,A3y,A4y)
%          fprintf('i: %d k: %d DY1: %g, DY2: %g , DY3: %g , DY4: %g \n',i,k,DY(1,k+1),DY(2,k+1),DY(3,k+1),DY(4,k+1))
...
%         end
        ...
        if i > stopping && hScaleFactor > 1
    %       if sum(ORD(i-stopping+1:i) < minORD) == 3
            if all(ORD(i-stopping+1:i) == ORD(i-1)) && ORD(i-1) <= minORD
                h = h*hScaleFactor;
            end
        end
